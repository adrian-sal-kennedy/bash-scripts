...
if [ -s "${file%.*}.m2v" ]
	then
	echo file "${file%.*}.m2v" exists! bailing out!
	exit 1;
fi 
...
if [ "x${h264_source}" == "x" ] || [ "x$2" != "x" ]; then
	vidopts="-c: copy -c:v mpeg2video -profile:v 4 -mbd rd -q:v 1 -qmin 1 -dc 10 -keyint_min 3 -bf 2 -maxrate 9400k -bufsize 1835008"
	...
	if [ "x${aMapString}" == "x" ]; then
		...
		filter_cplx="-filter_complex ${aFilter}alimiter=${ebur128}dB:-1dB:-1dB:5:75[aout];[aout]atrim=start=0.005[aout];[aout]channelsplit=channel_layout=stereo[left][right];[0:v]format=yuv420p,scale=w=1050:h=576:force_original_aspect_ratio=1,scale=w=704:h=576,pad=720:576:(ow-iw)/2:(oh-ih)/2,colormatrix=bt709:bt601,setsar=sar=64/45[vout]${trim}"
	fi
	vidmap="[vout]"
else
	vidopts="-c: copy -c:v mpeg2video -profile:v 4 -mbd rd -q:v 1 -qmin 1 -dc 10 -keyint_min 3 -bf 2 -maxrate 9400k -bufsize 1835008"
...
encodeCli+=' ${filter_cplx}
 -map ${vidmap} ${vidopts}
 "${file%.*}.m2v"
 -map "[left]" -c:a pcm_s16le -f wav -bitexact  "${file%.*}.FL.wav"
 -map "[right]" -c:a pcm_s16le -f wav -bitexact  "${file%.*}.FR.wav"'
