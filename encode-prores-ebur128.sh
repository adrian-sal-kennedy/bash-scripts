...
flag=$(echo $file | awk '{print match($0,"mezzanine.mov")}')

if [ $flag -gt 0 ] || [ -s "${file%.*}.mezzanine.mov" ]
	then
...
if [ "${file##*.}" == "mov" ]
	then
	encodeCli='$ffmpegBin -nostdin -enable_drefs 1 -i "${inFile}"'
else
...
if [ "x${h264_source}" == "x" ] || [ "x$2" != "x" ]; then
	...
	if [ "x${aMapString}" == "x" ]; then
		...
		filter_cplx="-filter_complex ${aFilter}alimiter=${ebur128}dB:-1dB:-1dB:5:75[aout];[aout]atrim=start=0.005[aout];[0:v]format=yuv422p10le,scale=out_range=tv[vout]${trim}"
	fi
	vidmap="[vout]"
else
	vidopts="-c:v prores"
...
encodeCli+=' ${filter_cplx}
 -map ${vidmap} ${vidopts}
 -map "[aout]" -map_metadata 0 -c:a pcm_s24le -c:d copy -timecode "${timecode}" -metadata "reel_name"="$reel_name" -metadata "timecode"="${timecode}" "${file%.*}.mezzanine.mov" 1>&2'
