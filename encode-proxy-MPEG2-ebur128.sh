...
if [ -s "${file%.*}.mezzanine.mov" ]
	then
	echo file "${file%.*}.mezzanine.mov" exists! bailing out!
	exit 1;
fi 
if [ -s "${file%.*}.mezzanine.MOS.mov" ]
	then
	echo file "${file%.*}.mezzanine.mov" exists! bailing out!
	exit 1;
fi 
...
if [ "x${h264_source}" == "x" ] || [ "x$2" != "x" ]; then
	vidopts="-c: copy -c:v mpeg2video -q:v 4"
	echo ${vidopts} >>test.txt
	if [ "x${aMapString}" == "x" ]; then
		echo not h264, no audio.
		filter_cplx="-filter_complex [0:v]format=yuv422p10le[vout]${trim_vidonly}"
	else
		echo not h264, has audio.
		filter_cplx="-filter_complex ${aFilter}alimiter=${ebur128}dB:-1dB:-1dB:5:75[aout];[aout]atrim=start=0.005[aout];[0:v]format=yuv422p10le[vout]${trim}"
	fi
	vidmap="[vout]"
else
	vidopts="-c: copy -c:v mpeg2video -q:v 4"
...
encodeCli+=' ${filter_cplx}
 -map ${vidmap} ${vidopts}
 -c:d copy "${file%.*}.mezzanine.MOS.mov"'
echo $encodeCli >test.txt
	else
encodeCli+=' ${filter_cplx}
 -map ${vidmap} ${vidopts}
 -map "[aout]" -c:a pcm_s24le -c:d copy "${file%.*}.mezzanine.mov"'
