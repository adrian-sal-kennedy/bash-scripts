...
if [ -s "${file%.*}.h264.mkv" ]
	then
	echo file "${file%.*}.h264.mkv" exists! bailing out!
	exit 1;
fi 
...
if [ "${file##*.}" == "mov" ]
	then
	encodeCli='$ffmpegBin -enable_drefs 1 -i "${inFile}"'
else
	encodeCli='$ffmpegBin -nostdin -i "${inFile}"'
fi

if [ "${file##*.}" == "vpy" ]
	then
	inFile="$(awk -F"'" '/source/ && !done {FS = "." ; print $2 ; done=1 }' "$1")"
	inFile="$(dirname "${file}")/${inFile}"
	if [ "${file##*.}" == "mov" ]
		then
...
if [ "x${h264_source}" == "x" ] || [ "x$2" != "x" ]; then
	vidopts="-c:v libx264 -tune film -crf 20 -preset fast"
	echo ${vidopts} >>test.txt
	if [ "x${aMapString}" == "x" ]; then
		echo not h264, no audio.
		filter_cplx="-filter_complex [0:v]format=yuv420p[vout]${trim_vidonly}"
	else
		echo not h264, has audio.
		filter_cplx="-filter_complex ${aFilter}alimiter=${ebur128}dB:-1dB:-1dB:5:75[aout];[aout]atrim=start=0.005[aout];[0:v]format=yuv420p[vout]${trim}"
	fi
	vidmap="[vout]"
else
	vidopts="-c:v libx264 -tune film -crf 20 -preset fast"
	...
	if [ "x${aMapString}" == "x" ]; then
		...
		echo h264, has audio.
		filter_cplx="-filter_complex ${aFilter}alimiter=${ebur128}dB:-1dB:-1dB:5:75[aout];[aout]atrim=start=0.005[aout]${trim}"
	fi
	vidmap="0:v"
fi
...
encodeCli+=' ${filter_cplx}
 -map ${vidmap} ${vidopts}
 -c:d copy "${file%.*}.h264.mkv"'
echo $encodeCli >test.txt
	else
encodeCli+=' ${filter_cplx}
 -map ${vidmap} ${vidopts}
 -map "[aout]" -c:a libopus -b:a 64k -c:d copy "${file%.*}.h264.mkv"'
...
#~/Sal/BATCHES/builds/ffmpeg-x264-10/bin/ffmpeg -y -i "World Peace and a Pony 1080p PRORES 422 HQ.mov" -an -c:v libx264 -preset ultrafast -tune grain -x264-params crf=2:bframes=2:keyint=12:interlaced=1:level=4.1:vbv-maxrate=25000:vbv-bufsize=25000 -vf "format=yuv420p" "World Peace and a Pony 1080p PRORES 422 HQ.mkv"
