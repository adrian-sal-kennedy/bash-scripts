...
if [ "${file##*.}" == "vpy" ]
	...
	if [ "${file##*.}" == "mov" ]
		...
		encodeCli='vspipe "$file" - --y4m | $ffmpegBin -i - -enable_drefs 1 -i "${inFile}" '
	else
		#encodeCli='vspipe "$file" - --y4m | $ffmpegBin -y -i - -i "${inFile}"'
		encodeCli='vspipe "$file" - --y4m | $ffmpegBin -i - -i "${inFile}"'
	fi
	echo $inFile
	echo $encodeCli
fi
...
if [ "x${h264_source}" == "x" ] || [ "x$2" != "x" ]; then
	...
	if [ "x${aMapString}" == "x" ]; then
		...
		echo h264, has audio.
		filter_cplx="-filter_complex ${aFilter}asplit=2[aout][aout2];[aout]alimiter=${ebur128}dB:-1dB:-1dB:5:75[aout];[aout]atrim=start=0.005[aout]${trim}"
	fi
	vidmap="0:v"
fi
...
encodeCli+=' ${filter_cplx}
 ...
 -c:d copy -timecode "${timecode}" -metadata "handler_name"="Apple Video Media Handler" -metadata "reel_name"="$reel_name" -metadata "timecode"="${timecode}" "${file%.*}.mezzanine.mov" 1>&2'
#echo "$encodeCli" >test.txt
	else
encodeCli+=' ${filter_cplx}
 -map ${vidmap} ${vidopts}
 -map "[aout]" -map_metadata 0 -c:a pcm_s24le -c:d copy -timecode "${timecode}" -metadata "handler_name"="Apple Video Media Handler" -metadata "reel_name"="$reel_name" -metadata "timecode"="${timecode}" "${file%.*}.mezzanine.mov" 1>&2'
